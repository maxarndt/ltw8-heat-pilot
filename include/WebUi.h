...
<script>
const q=s=>document.querySelector(s),qa=s=>document.querySelectorAll(s);let busy=false,loading=false,currentMode='';
const states={disabled:'Deaktiviert',manual_control:'Manuelle Steuerung',monitoring:'Bereit',heating:'Heizbetrieb',pump_overrun:'Pumpennachlauf',temperature_hold:'Zieltemperatur erreicht',waiting_for_data:'Warte auf Energiedaten',waiting_for_temperature:'Warte auf Temperatur',temperature_fault:'Temperaturfehler',temperature_configuration_fault:'Sensorkonfiguration fehlerhaft',fault:'Fehler',starting:'Startet'};
...
function alertMessage(s){if(!s.temperature_configuration_valid)return 'Temperatursensoren entsprechen nicht der Konfiguration – Heizen ist gesperrt.';if(!s.output_driver_healthy)return 'Ausgangstreiber nicht verfügbar.';if(s.temperature_fault)return 'Temperaturmessung fehlerhaft – Heizen ist gesperrt.';if(!s.temperature_valid)return 'Keine gültige Temperaturmessung.';if(!s.measurements_valid&&s.mode==='automatic')return 'Automatik wartet auf aktuelle Energie- oder Batteriedaten.';return ''}
...
const sensors=q('#temperatures');sensors.replaceChildren();(s.temperature_sensors||[]).forEach((t,i)=>{const r=document.createElement('div');r.className='row';const n=document.createElement('span');n.textContent=(t.label||'Unbekannt')+' · '+t.address;const v=document.createElement('strong');v.textContent=t.valid?number(t.temperature_c,1)+' °C':(t.configured?'Ungültig':'Nicht konfiguriert');r.append(n,v);sensors.append(r)});
...
async function load(){if(busy||loading)return;loading=true;const controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),1500);try{const r=await fetch('/api/v1/status',{cache:'no-store',signal:controller.signal});if(!r.ok)throw Error();render(await r.json())}catch(e){const c=q('#connection');c.className='connection offline';c.querySelector('span').textContent='Offline';q('#alert').textContent='Verbindung zum Heat Pilot unterbrochen.';q('#alert').classList.add('show')}finally{clearTimeout(timeout);loading=false}}
